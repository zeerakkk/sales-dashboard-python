...
app.layout = dbc.Container([
    html.H1("📈 Sales Dashboard", className="text-center mt-4 mb-4"),
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.Div(id="total-sales-card", className="fs-4 fw-bold")
                ])
            ], color="primary", inverse=True, className="text-center")
        ], width=12, md=6)
...
def update_dashboard(selected_category):
    # Bar chart
    bar_fig = px.bar(df, x="month", y=selected_category,
                     title=f"Monthly Sales - {selected_category}",
                     color="month", text=selected_category)
    bar_fig.update_layout(showlegend=False, yaxis_title="Sales ($)", template="plotly_white")

    # Line chart
    line_fig = px.line(df, x="month", y=selected_category,
                       markers=True, title=f"Sales Trend - {selected_category}")
